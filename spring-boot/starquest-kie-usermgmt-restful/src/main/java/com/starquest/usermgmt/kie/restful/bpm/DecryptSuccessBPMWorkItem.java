...
@Component
public class DecryptSuccessBPMWorkItem implements ProcessBPMWorkItem {

private final String workItemName = SqEndPoints.PASSWORD_DECRYPT_SUCCESS_ENDPOINT.name();//change this to enum later
	...
	public ResponseEntity<UserVo> callRestfulEndPoint(WorkItem workItem) throws Exception {
		...
		return restTeamplate.exchange(
				sqBpmConfig.getPasswordDecryptSuccessFlowEndPointDetails().getUrl(),HttpMethod.POST, entity, UserVo.class);
	}
	...
	public void prepareForNextStep(UserVo objToPass,  Boolean nextStep, WorkItem workItem,
        ...
        String workFlowStep = sqBpmConfig.getLoginCredentialsFailFlow();
        if(nextStep){
        	workFlowStep = sqBpmConfig.getLoginCredentialsSucessFlow();
        }
